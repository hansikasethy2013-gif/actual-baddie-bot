...
client.on('messageCreate', async (message) => {
  ...
  // --- COMMAND HANDLING ---
  // Using "return" after each command ensures the bot stops thinking and doesn't reply twice!
  ...
  if (content.startsWith('$profile')) {
    ...
    const rank = rep > 50 ? "Iconic Legend 👑" : "Broke & Basic 🤢";
  ...
  if (content.startsWith('$tea')) return message.reply(teaVault[Math.floor(Math.random() * teaVault.length)]);
  if (content.startsWith('$checkrep')) return message.reply(`Reputation: ${userReputation[userId]} 💅`);

  // --- PASSIVE REACTIONS (ONLY RUNS IF NO COMMAND WAS USED) ---
